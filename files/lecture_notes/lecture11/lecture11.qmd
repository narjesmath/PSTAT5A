...
format: 
  revealjs:
    ...
    preview-links: auto
    logo: ""
    footer: "Hypothesis Testing Lecture"
    transition: slide
...
- **Alternative Hypothesis (H₁ or Hₐ):** The research claim we want to test
  - Usually includes "≠", "<", or ">"
...
- H₀: μ = 75 (no improvement, scores stay the same)
- H₁: μ > 75 (scores improve with new method)
:::
...
axes[1].text(0, 0.35, 'H₀: μ ≥ μ₀', ha='center', fontsize=10,
             bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7))
...
axes[2].text(0, 0.35, 'H₀: μ ≤ μ₀', ha='center', fontsize=10,
             bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7))
...
- **α = 0.01:** More conservative, when Type I errors are costly
- **α = 0.10:** Less conservative, when Type II errors are costly
:::
...
- ❌ "Accept H₀" or "Prove H₁"
- ✅ "Evidence suggests..." or "Data supports..."
- ❌ "H₁ is true" or "H₀ is false"
:::
...
error_data = {
    ...
    'H₀ is True (Reality)': ['Type I Error\n(α = P(Type I))', 'Correct Decision\n(1 - α)'],
    'H₀ is False (Reality)': ['Correct Decision\n(1 - β = Power)', 'Type II Error\n(β = P(Type II))']
}
...
table_data = [
    ...
    ['Decision: Reject H₀', 'Type I Error\n(α = P(Type I))\n"False Positive"', 'Correct Decision\n(1 - β = Power)\n"True Positive"'],
    ['Decision: Fail to Reject H₀', 'Correct Decision\n(1 - α)\n"True Negative"', 'Type II Error\n(β = P(Type II))\n"False Negative"']
]
...
axes[0].set_title('Power vs Sample Size\n(Effect Size = 0.5, α = 0.05)', fontweight='bold')
...
- H₀: μ = 75 (new method doesn't improve scores)
...
ax1.axvline(mu_0, color='orange', linewidth=3, linestyle='--', label=f'H₀: μ = {mu_0}')
...
# Hypotheses
print("\nHypotheses:")
print("H₀: μ_A = μ_B (no difference between methods)")
print("H₁: μ_A ≠ μ_B (there is a difference)")
...
if p_val_2samp <= alpha:
    print(f"\nDecision: Reject H₀ (p = {p_val_2samp:.4f} ≤ {alpha})")
    print("Conclusion: There is a significant difference between the two teaching methods.")
else:
    print(f"\nDecision: Fail to reject H₀ (p = {p_val_2samp:.4f} > {alpha})")
...
test_data = {
    ...
    'Purpose': ['Compare sample mean to known value', 'Compare means of two groups', 
                'Compare paired observations', 'Compare sample mean (known σ)',
...
**Questions:**
1. Set up appropriate hypotheses (use α = 0.05)
...
# 1. Hypotheses
print("1. Hypotheses:")
print("   H₀: μ = 5 (average wait time is 5 minutes)")
print("   H₁: μ ≠ 5 (average wait time is different from 5 minutes)")
...
print(f"\n3. Calculations:")
print(f"   t = ({sample_mean} - {claimed_mean}) / ({sample_std} / √{n}) = {t_stat:.3f}")
print(f"   df = {n} - 1 = {df}")
print(f"   p-value = {p_value:.4f}")
...
print(f"   {decision} (p = {p_value:.4f}, α = {alpha})")
...
5. **Choosing α after seeing results**
...
ax1.axvline(100, color='blue', linewidth=3, linestyle='--', label='H₀: μ = 100')
...
ax2.axvline(100, color='blue', linewidth=3, linestyle='--', label='H₀: μ = 100')
...
::: {.incremental}
1. **Start with skepticism** (assume H₀ is true)
2. **Collect evidence** (sample data)
3. **Quantify surprise** (how unusual is this result if H₀ were true?)
4. **Make a decision** (is the evidence strong enough to reject H₀?)
