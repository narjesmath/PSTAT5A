...
fig.add_hline(y=0.05, line_dash="dash", line_color=colors['danger'], 
              line_width=2, annotation_text="α = 0.05", row=1, col=2)
...
misconceptions_data = [
    ["❌ WRONG", "✅ CORRECT"],
    ["P-value = P(H₀ is true)", "P-value = P(data | H₀ is true)"],
    ["P-value = probability of error", "P-value assumes H₀ is true"],
    ["1-p = P(H₁ is true)", "P-value is NOT posterior probability"],
    ["Smaller p = bigger effect", "P-value ≠ effect size"],
    ["p > 0.05 means no effect", "p > 0.05 means insufficient evidence"]
]

fig.add_trace(go.Table(
    header=dict(
        values=["❌ Common Misconception", "✅ Correct Understanding"],
...
p_ranges = ['p > 0.10', '0.05 < p ≤ 0.10', '0.01 < p ≤ 0.05', 'p ≤ 0.01']
...
fig.add_vline(x=current_average, line_dash="dash", line_color=colors['warning'], 
              line_width=3, annotation_text=f"H₀: μ = {current_average}", row=1, col=1)
...
test_info = [
    ...
    ["Group 1 (Method A)", f"n₁ = {n1}, x̄₁ = {mean1:.2f}, s₁ = {s1:.2f}"],
    ["Group 2 (Method B)", f"n₂ = {n2}, x̄₂ = {mean2:.2f}, s₂ = {s2:.2f}"],
...
if p_val_2samp <= alpha:
    print(f"\n✅ DECISION: Reject H₀")
    print(f"📊 CONCLUSION: There is statistically significant evidence (p = {p_val_2samp:.4f}) of a difference between teaching methods.")
    print(f"💡 PRACTICAL IMPACT: Method B appears to be {mean2 - mean1:.1f} points better than Method A.")
else:
    print(f"\n❌ DECISION: Fail to reject H₀")
    print(f"📊 CONCLUSION: No statistically significant difference between methods (p = {p_val_2samp:.4f}).")
...
print(f"🎯 EFFECT SIZE: {effect_interp} effect (d = {abs(cohens_d_2samp):.3f})")
```
...
fig.add_vline(x=100, line_dash="dash", line_color=colors['warning'], 
              line_width=3, annotation_text="H₀: μ = 100", row=1, col=1)
...
fig.add_vline(x=100, line_dash="dash", line_color=colors['warning'], 
              line_width=3, annotation_text="H₀: μ = 100", row=1, col=2)
...
**Next class:** Multiple testing and ANOVA! 📊
